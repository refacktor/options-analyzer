...
public class Utils {
	...
	public static List<List<OptionQuote>> getOptionPairs(JsonNode rootNode, int partitionSize) {
		...
		rootNode.withArray("options").elements().forEachRemaining(node -> {
			...
			optionChainResponseNode.withArray("OptionPair").elements().forEachRemaining(optionPairNode -> {
				...
				results.add(OptionQuote.from(expiration, uniquePair, callNode));
				results.add(OptionQuote.from(expiration, uniquePair, putNode));
			});
		});
